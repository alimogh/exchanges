 ...
 * EXX API client.
 *
 * Each IP can send maximum of 1000 https requests within 1 minute. If exceeds 1000 requests, the system will automatically block the IP for one hour.
 * After an hour, the IP will be automatically unlocked.
 *
 */

const request = require('request');
const debug = require('debug')('cointrage:exchanges:exx');

const API_URL = 'https://api.exx.com/data/v1';
...
const getOrderBook = (market, ticker) => new Promise((resolve, reject) => {
    ...
    const url = `${API_URL}/depth?currency=${ticker.toLowerCase()}_${market.toLowerCase()}`;
